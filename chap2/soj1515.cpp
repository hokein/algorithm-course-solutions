...
state do_operation(const state& s, int mode) {
  ...
  switch (mode) {
    case 0: // operation A.
      return state(x%100*100+x/100,
                   y%100*100+y/100);
    case 1: // operation B.
      return state(x%1000*10+x/1000,
                   y%1000*10+y/1000);
    case 2: // operation C.
      return state(x/1000*1000 + (x/10)%10*100 + (y/10)%10*10 + x%10, 
                   y/1000*1000 + (x/100)%10*100 + (y/100)%10*10 + y%10);
  }
  return s;
}
